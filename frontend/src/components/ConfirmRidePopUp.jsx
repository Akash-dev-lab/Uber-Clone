            ...
            <div className='flex gap-2 justify-between flex-col items-center'>
                ...
                <div className='mt-6 w-full'>
                    <form onSubmit={(e)=>{
                        ...
                        <input value={otp} onChange={(e)=>{setOtp(e.target.value)}} type="text" placeholder='Enter OTP' className='bg-[#eeeeee] px-6 py-4 text-lg font-mono rounded-lg w-full mt-3' />
                        <Link to='/captain-riding' className='w-full mt-5 flex justify-center text-lg bg-green-600 text-white font-semibold p-3 rounded-lg'>Confirm</Link>

                        <button onClick={() => {
                            props.setRidePopupPanel(false)
                            props.setConfirmRidePopupPanel(false)
                        }} className='w-full mt-2 bg-red-600 text-white font-semibold text-lg p-3 rounded-lg'>Cancel</button>
                    </form>
                </div>
            </div>
        </div>
